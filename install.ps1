...
if ([string]::IsNullOrWhiteSpace($Action)) {
    ...
    Write-Host "  8) mcp" -ForegroundColor Gray
    Write-Host "  9) ssh" -ForegroundColor Gray
    Write-Host "  10) hyperv" -ForegroundColor Gray
    Write-Host "  11) wsl" -ForegroundColor Gray
    Write-Host "  12) wsl-ubuntu" -ForegroundColor Gray
    $choice = Read-Host "请输入序号 (1-12)"
    switch ($choice) {
        "1" { $Action = "base" }
        "2" { $Action = "choco" }
        "3" { $Action = "scoop" }
...
switch ($Action) {
    ...
    default {
        ...
        Write-Host "可用的 action: base, choco, scoop, node, claude, codex, gemini, mcp, ssh, hyperv, wsl, wsl-ubuntu" -ForegroundColor Yellow
        exit 1
    }
}